    ...
    io.on('connection', function (socket) {
      ...
      socket.on('computation_id', function (msg) {
        ...
        if (party_id != null) { // party_id is given, check validity
          // party_id is not a number or is outside of range
          if (isNaN(party_id) || party_id <= 0 || party_id > party_count) {
            io.to(socket.id).emit('error', 'Invalid party ID');
            return;
          } else if (spare_party_ids[computation_id] != null && spare_party_ids[computation_id].remove(party_id) === false) { // will reserve party_id if spare
      ...
      socket.on('open', function (msg, callback) {
        ...
        if (jiff.logs) {
          console.log('open from ' + computation_id + '-' + from_id + ' : ' + msgs);
        }
      ...
      socket.on('custom', function (msg, callback) {
        ...
        if (jiff.logs) {
//          console.log('custom from ' + computation_id + '-' + from_id + ' : ' + msg.substring(1,30));
        }
    ...
    jiff.request_triplet_share = function (msg, computation_id, from_id) {
      ...
      if (jiff.logs) {
        ...
        console.log('triplet ' + triplet_id + ' from ' + computation_id + '-' + from_id + ':: ' + msg);
      }
    ...
    jiff.request_number_share = function (msg, computation_id, from_id) {
      ...
      if (jiff.logs) {
        console.log('number ' + base_number_id + ' from ' + computation_id + '-' + from_id + ':: ' + msg); //JSON.stringify(msg));
      }
